...
# Redis expiration time - 4 hours (optimal: no egress cost + lower memory usage)
COMPANY_INDEX_TTL = 3600 * 4

class Command(BaseCommand):
    help = 'DEPRECATED: Use build_company_postgresql instead. This Redis approach is no longer used.'
    ...
    def handle(self, *args, **options):
        ...
        for company_name in unique_companies:
            ...
            try:
                ...
                # Build clean HTML for this company (simplified - just name and link)
                # Use proper URL format: /companies/{URL_encoded_name}/?sort_by=location&sort_order=asc
                from urllib.parse import quote
                encoded_company_name = quote(company_name)
                company_html = f"""
                <div>
                    <strong><a href="/companies/{encoded_company_name}/?sort_by=location&sort_order=asc">{company_name}</a></strong>
                </div>
                """
                ...
                company_index[normalized_name] = {
                    ...
                    "years": sorted_years,
                    "url": f"/company-list/{company_url_name}/",
