...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--light',
            action='store_true',
            help='Light rebuild - minimal egress for deployment safety'
        )
        parser.add_argument(
            '--check-changes',
            action='store_true',
            help='Only rebuild if data has changed since last run'
        )
    ...
    def handle(self, *args, **options):
        ...
        # Light mode: minimal egress for deployment safety
        if light:
            self.stdout.write(self.style.WARNING('Light mode: minimal rebuild to reduce egress'))
            # In light mode, just verify cache exists and skip heavy operations
            existing_mapping = cache.get(cache_key, version=cache_version)
            if existing_mapping:
                self.stdout.write(self.style.SUCCESS('Location mapping exists - skipping rebuild in light mode'))
                return
            else:
                self.stdout.write(self.style.WARNING('No existing mapping found - light mode cannot help'))
        
        # Check changes mode: only rebuild if data has changed
        if check_changes and not force:
            from django.db import connection
            with connection.cursor() as cursor:
                cursor.execute("SELECT COUNT(*) FROM checker_component WHERE updated_at > NOW() - INTERVAL '24 hours'")
                recent_changes = cursor.fetchone()[0]
                if recent_changes == 0:
                    self.stdout.write(self.style.SUCCESS('No recent data changes - skipping rebuild'))
                    return
                else:
                    self.stdout.write(self.style.SUCCESS(f'Found {recent_changes} recent changes - proceeding with rebuild'))
