...
def search_components(request):
    ...
    # --- Preserve the per_page value from the request, with 50 as default --- 
...
# Removed duplicate component_detail function - see line 1666 for the actual implementation
...
# --- DEPRECATED: Legacy Component-based search function ---
# This function was replaced by LocationGroup-based search in search_map_view_simple
# Causes 3-5 second delays due to slow Component table queries
# TODO: Remove this entire function (200+ lines) in next cleanup
def search_DEPRECATED(request):
    query = request.GET.get('q', '').strip()
    start_time = time.time()
    ...
    try:
        ...
        if query and total_count == 0 and len(company_list) == 0:
...
@monitor_api
@map_access_required
def search_map_view(request):
    """Search view that displays results in a map interface"""
    from .services.component_search_optimized_v2 import search_components_optimized_v2
    from django.shortcuts import render
    
    # Log the request
    logger.info(f"search_map_view called with query: {request.GET.get('q', '')}")
    
    # Get the query
    query = request.GET.get('q', '').strip()
    
    if not query:
        # Render simple search page if no query
        context = {
            'query': '',
            'google_maps_api_key': settings.GOOGLE_MAPS_API_KEY if hasattr(settings, 'GOOGLE_MAPS_API_KEY') else '',
            'user': request.user,
        }
        return render(request, 'checker/search_map_view_styled.html', context)
    
    # Get the search results from optimized v2
    # We'll call it as a function and extract the context
    from django.test import RequestFactory
    factory = RequestFactory()
    
    # Create a modified request to prevent redirect
    modified_GET = request.GET.copy()
    modified_GET['suppress_map'] = 'true'
    fake_request = factory.get('/', modified_GET)
    fake_request.user = request.user
    fake_request.session = request.session
    
    # Get the response from optimized v2
    response = search_components_optimized_v2(fake_request)
    
    # Extract context from the response
    if hasattr(response, 'context_data'):
        context = response.context_data
    else:
        # Parse the rendered content to extract the data we need
        # For now, just call the service directly and render our template
        from .services.location_search import search_locations
        from .services.company_search import _perform_company_search, get_cmu_dataframe
        ...
        try:
            ...
            if cmu_df is not None:
                ...
                if len(matching_records) > 0:
                    # Format company links
                    for _, row in matching_records.head(5).iterrows():
                        company_name = row.get("Full Name", "")
                        company_id = normalize(company_name)
                        link_html = f'<a href="/company/{company_id}/" class="company-link">{company_name}</a>'
                        company_links.append(link_html)
        except Exception as e:
            logger.error(f"Error in company search: {e}")
        
        # Search for locations
        location_result = search_locations(
            query=query,
            page=page,
            per_page=per_page,
            sort_by=sort_by,
        ...
        # Enhance location groups with coordinate data
        location_groups = location_result['location_groups']
        
        # For each location group, we need to get coordinates
        # We'll use the first component's coordinates as a representative
        from .models import Component
        for group in location_groups:
            if 'latitude' not in group or 'longitude' not in group:
                # Try to get coordinates from the first component with this location
                try:
                    first_component = Component.objects.filter(
                    ...
                    if first_component:
                        group['latitude'] = float(first_component.latitude)
                        group['longitude'] = float(first_component.longitude)
                except Exception as e:
                    logger.error(f"Error getting coordinates for location {group['location']}: {e}")
        
        context = {
            'query': query,
            'company_links': company_links,
            'location_groups': location_groups,
            'page_obj': location_result['page_obj'],
            'total_components': location_result['total_components'],
            'component_count': location_result['total_components'],
            'google_maps_api_key': settings.GOOGLE_MAPS_API_KEY if hasattr(settings, 'GOOGLE_MAPS_API_KEY') else '',
        }
    ...
    # Render with our styled template
    return render(request, 'checker/search_map_view_styled.html', context)
...
def derated_capacity_list(request):
    ...
    # Generate cache key
    cache_key = f"derated_capacity_list_p{page}_s{sort_order_param}_pp{per_page}"
    
    # Try cache first
    cached_response = cache.get(cache_key)
    if cached_response:
        logger.info(f"✅ derated_capacity_list cache HIT for page {page}")
        return cached_response
    
    logger.info(f"❌ derated_capacity_list cache MISS for page {page} - building...")
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    ...
    logger.info(f"✅ Cached derated_capacity_list page {page}")
...
def company_capacity_list(request):
    ...
    # Generate cache key
    cache_key = f"company_capacity_list_p{page}_s{sort_order_param}_pp{per_page}"
    
    # Try cache first
    cached_response = cache.get(cache_key)
    if cached_response:
        logger.info(f"✅ company_capacity_list cache HIT for page {page}")
        return cached_response
    
    logger.info(f"❌ company_capacity_list cache MISS for page {page} - building...")
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    ...
    response = render(request, 'checker/company_capacity_list.html', context)


    cache.set(cache_key, response, CACHE_TTL)


    logger.info(f"✅ Cached company_capacity_list page {page}")


    return response

@require_http_methods(["GET"])
def company_component_count_list_view(request):
    """Display a paginated list of all companies, ranked by their component count."""
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    from django.core.cache import cache  # Added for caching
    ...
    # Generate cache key
    cache_key = f"company_component_count_list_view_p{page}_s{sort_order_param}_pp{per_page}"
    
    # Try cache first
    cached_response = cache.get(cache_key)
    if cached_response:
        logger.info(f"✅ company_component_count_list_view cache HIT for page {page}")
        return cached_response
    
    logger.info(f"❌ company_component_count_list_view cache MISS for page {page} - building...")
    sort_order = request.GET.get('sort', 'desc') # Default to descending
    page_number = request.GET.get('page', 1)

    # Annotate each company with its total component count
    # We need to count distinct components linked to each company.
    # Assuming Component model has a ForeignKey to a Company-like model or a company_name field.
    # If using a direct relation (e.g., component.company_info), it would be:
    # companies_with_counts = CompanyInfo.objects.annotate(component_count=Count('component'))
    # If using component.company_name and need to group by that:
    companies_with_counts = Component.objects.values('company_name') \
                                .annotate(component_count=Count('id')) \
                                .exclude(company_name__isnull=True).exclude(company_name__exact='')

    if sort_order == 'asc':
        ordered_companies = companies_with_counts.order_by('component_count', 'company_name')
        current_sort_label = "Smallest First"
        new_sort_order = 'desc'
    else: # Default to descending
        ordered_companies = companies_with_counts.order_by('-component_count', 'company_name')
        current_sort_label = "Largest First"
        new_sort_order = 'asc'

    paginator = Paginator(ordered_companies, 50) # Show 50 companies per page
    ...
    context = {
        ...
        'page_title': "Companies Ranked by Component Count",
        'header_title': "Companies Ranked by Component Count",
        'metric_name': "Components", # For the template to display 'X Components'
        'is_active_page': 'statistics' # For nav highlighting
    }

    # Cache the response before returning


    response = render(request, 'checker/company_component_count_list.html', context)


    cache.set(cache_key, response, CACHE_TTL)


    logger.info(f"✅ Cached company_component_count_list_view page {page}")
...
def technology_list_view(request):
    ...
    # Generate cache key
    cache_key = f"technology_list_view_p{page}_s{sort_order_param}_pp{per_page}"
    
    # Try cache first
    cached_response = cache.get(cache_key)
    if cached_response:
        logger.info(f"✅ technology_list_view cache HIT for page {page}")
        return cached_response
    
    logger.info(f"❌ technology_list_view cache MISS for page {page} - building...")
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    ...
    response = render(request, 'checker/technology_list.html', context)


    cache.set(cache_key, response, CACHE_TTL)


    logger.info(f"✅ Cached technology_list_view page {page}")


    return response

@require_http_methods(["GET"])
def technology_capacity_list_view(request):
    """Displays a full, paginated list of technologies ranked by total De-rated Capacity."""
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    from django.core.cache import cache  # Added for caching
    ...
    # Generate cache key
    cache_key = f"technology_capacity_list_view_p{page}_s{sort_order_param}_pp{per_page}"
    
    # Try cache first
    cached_response = cache.get(cache_key)
    if cached_response:
        logger.info(f"✅ technology_capacity_list_view cache HIT for page {page}")
        return cached_response
    
    logger.info(f"❌ technology_capacity_list_view cache MISS for page {page} - building...")
    from django.core.paginator import Paginator, EmptyPage, PageNotAnInteger
    from django.db.models import Sum, Value, FloatField # Import FloatField
    from django.db.models.functions import Coalesce
    import logging
    import time
    ...
    per_page = 50 # Technologies per page
    error_message = None
    total_count = 0
    tech_page = None
    paginator = None
    ...
    try:
        # Calculate the grand total capacity of all technologies
        grand_total_capacity_result = Component.objects.exclude(derated_capacity_mw__isnull=True) \
                                               .aggregate(grand_total=Coalesce(Sum('derated_capacity_mw', output_field=FloatField()), Value(0.0, output_field=FloatField())))
        grand_total_capacity = grand_total_capacity_result['grand_total']

        # Query technologies ranked by total capacity
        tech_queryset = Component.objects.exclude(technology__isnull=True) \
                                   .exclude(technology='') \
                                   .exclude(derated_capacity_mw__isnull=True) \
                                   .values('technology') \
                                   .annotate(total_capacity=Sum('derated_capacity_mw', output_field=FloatField())) \
                                   .order_by(db_sort_field)
                                    
        total_count = tech_queryset.count()
        logger.info(f"Found {total_count} technologies with de-rated capacity, sorting by {db_sort_field}. Grand total capacity: {grand_total_capacity}")
        
        paginator = Paginator(tech_queryset, per_page)
        try:
            tech_page_qs = paginator.page(page)
        except PageNotAnInteger:
            tech_page_qs = paginator.page(1)
        except EmptyPage:
            tech_page_qs = paginator.page(paginator.num_pages)

        # Prepare data for the template, including percentage
        processed_tech_list = []
        for tech_data in tech_page_qs.object_list:
            percentage = 0
            if grand_total_capacity > 0 and tech_data['total_capacity'] is not None:
                percentage = (tech_data['total_capacity'] / grand_total_capacity) * 100
            tech_data['percentage'] = percentage
            processed_tech_list.append(tech_data)
        
        # Create a new Page object with the processed list
        # This is a bit of a workaround to inject 'percentage' into the page_obj's object_list
        # For simplicity, we'll pass processed_tech_list directly to context as 'object_list'
        # and the original tech_page_qs as 'page_obj' for pagination controls.
        tech_page = tech_page_qs # Keep for pagination attributes
            
    except Exception as e:
        logger.error(f"Error processing technology capacity list: {e}")
        error_message = f"Error processing technology list: {e}"
        tech_page = None 
        total_count = 0
        processed_tech_list = [] # Ensure it's initialized
        if paginator is None: # Ensure paginator is initialized even on error
            paginator = Paginator([], per_page) 
        tech_page = paginator.page(1) # Initialize tech_page for pagination attributes
    
    api_time = time.time() - start_time
    
    context = {
        "page_obj": tech_page, # Original page object for pagination
        "object_list": processed_tech_list, # List with percentages
        "paginator": paginator,
        "total_count": total_count,
        "api_time": api_time,
        "error": error_message,
        "sort_order": sort_order_param, # Pass sort order to template
        "page_title": "Technologies by Total Capacity (MW)",
        "header_title": "Technologies Ranked by Total Capacity (MW)",
        "metric_name": "MW Total Capacity", 
        # Pagination context variables
        "page": tech_page.number if tech_page else 1, 
        "per_page": per_page,
        "total_pages": paginator.num_pages if paginator else 0,
        "has_prev": tech_page.has_previous() if tech_page else False,
        "has_next": tech_page.has_next() if tech_page else False,
        "page_range": paginator.get_elided_page_range(number=tech_page.number if tech_page else 1, on_each_side=1, on_ends=1) if paginator else []
    }

    # Cache the response before returning


    response = render(request, 'checker/technology_capacity_list.html', context)


    cache.set(cache_key, response, CACHE_TTL)


    logger.info(f"✅ Cached technology_capacity_list_view page {page}")
...
def map_view(request):
    ...
    # CM period years - properly structured for JavaScript
    cm_period_years = {
        'future': [str(year) for year in range(2024, 2029)],  # 2024-2028
        'past': [str(year) for year in range(2016, 2024)]     # 2016-2023
    }
    ...
    context = {
        ...
        'total_count': total_count,
        'cm_period_years': cm_period_years,
        'mode': 'technology_selection',
        'search_query': '',  # Add empty search query for template compatibility
    }
    
    return render(request, 'checker/map_technology.html', context)

@map_access_required
def technology_map_results_view(request):
    """Render the technology selection map page"""
    # Get technology options for filters (same as main map)
    technologies = Component.objects.exclude(technology__isnull=True) \
                               .exclude(technology='') \
                               .values_list('technology', flat=True) \
                               .distinct() \
    ...
    # Get company options for filters (same as main map)
    companies = Component.objects.exclude(company_name__isnull=True) \
                                .exclude(company_name='') \
                                .values_list('company_name', flat=True) \
                                .distinct() \
                                .order_by('company_name')[:100]
...
@map_access_required
def company_map_results_view(request):
    """Render the company selection map page"""
    # Get same context as main map
    technologies = Component.objects.exclude(technology__isnull=True) \
                               .exclude(technology='') \
                               .values_list('technology', flat=True) \
                               .distinct() \
    ...
    context = {
        ...
        'geocoded_count': geocoded_count,
        'total_count': total_count,
        'mode': 'company_selection',
    }
    
    return render(request, 'checker/map_company.html', context)
...
def map_data_api(request):
    ...
    # Add 'q' to relevant_params for cache key if it's part of the filtering
    search_query_param = params.get('q', '') # Get search query for cache key
    ...
    if search_query_param and search_query_param.upper().strip() == 'SW11':
        # Special case for SW11 - include a timestamp to avoid stale cache
        import time
        current_time = int(time.time() / 300)  # Changes every 5 minutes
        cache_key = f"map_data_sw11_{current_time}"
        print(f"DEBUG: Using special SW11 cache key with timestamp: '{cache_key}'")
    else:
        ...
        if search_query_param: # Add q to cache key if present
            relevant_params.append('q')
    ...
    search_query_for_early_exit = request.GET.get('q', '')
    
    # Also check for search_query parameter as a fallback
    if not search_query_for_early_exit:
        search_query_for_early_exit = request.GET.get('search_query', '')
    
    # Log all parameters to help with debugging
    print(f"MAP DATA API: technology_param_exists={technology_param_exists}, company_filter='{company_filter}', search_query='{search_query_for_early_exit}'")
    print(f"MAP DATA API: All parameters: {dict(request.GET.items())}")
    
    # Only require technology if no company is specified AND no search query is present
    if not technology_param_exists and not company_filter and not search_query_for_early_exit:
        empty_time = record_stage_time("early_exit")
        print(f"No technology, company filter, or search query provided. Returning empty. (in {empty_time:.3f}s)")
        empty_response = {
    ...
    # For normal operation, only include geocoded components
    # But for search queries, include all components and handle geocoding later
    search_query = request.GET.get('q', None) or request.GET.get('search_query', None)
    
    # Always start with all components for search queries
    base_query = Component.objects.all()
    print(f"MAP DATA API: Starting with all components - will filter later")
    
    # --- START: Apply Text Search Query (if provided) ---
    # We already have the search query from above
    
    if search_query:
        print(f"MAP DATA API: Processing search query: '{search_query}' (type: {type(search_query)})")
        
        # For SW11 search, use outward_code directly - fixed approach
        if search_query.upper().strip() == 'SW11':
            print(f"MAP DATA API: SW11 search detected! Using outward_code filter")
            base_query = base_query.filter(outward_code__iexact='SW11')
            
            # Check if we found components
            sw11_count = base_query.count()
            print(f"MAP DATA API: Found {sw11_count} components with outward_code='SW11'")
            
            # If we somehow found no results, fall back to normal search
            if sw11_count == 0:
                print(f"MAP DATA API: No components found with outward_code='SW11', falling back to text search")
                base_query = Component.objects.all()
            ...
            # For other searches, use standard text search approach
            ...
            for term in search_terms:
                ...
                query_filter_text_search &= term_filter # AND logic for terms
            base_query = base_query.filter(query_filter_text_search)
            
        print(f"Applied search filter: '{search_query}'")
    
    # For non-search operations, filter for geocoded components only
    elif not search_query:
        print(f"MAP DATA API: No search query - filtering for geocoded components only")
        base_query = base_query.filter(geocoded=True, latitude__isnull=False, longitude__isnull=False)
        
    record_stage_time("text_search_filter")
    # --- END: Apply Text Search Query ---
    
    # Add special debug for SW11 search
    if search_query and search_query.lower() == 'sw11':
        sw11_count = base_query.count()
        print(f"MAP DATA API: After applying filters, found {sw11_count} SW11 components")
        
        # Log some example components for SW11
        if sw11_count > 0:
            sample = base_query.first()
            print(f"MAP DATA API: Sample SW11 component: {sample.cmu_id} - {sample.company_name} - {sample.location}")
            # Check geocoding status
            geocoded_count = base_query.filter(geocoded=True, latitude__isnull=False, longitude__isnull=False).count()
            print(f"MAP DATA API: Of those, {geocoded_count} components are geocoded")
        else:
            print("MAP DATA API: No components found for SW11 search query")
    ...
    if specific_year_requested:
        ...
        # Apply filter based on selected CM period using LocationGroup is_active field
        # This approach respects the database-level active/inactive status
        
        if cm_period == 'historical':
            # Historical: Components at inactive locations (is_active=False)
            # Get location names where is_active=False
            inactive_locations = LocationGroup.objects.filter(is_active=False).values_list('location', flat=True)
            base_query = base_query.filter(location__in=inactive_locations)
            print(f"Applied historical CM period filter: showing {inactive_locations.count()} inactive locations")
        else:
            # Future/Active: Components at active locations (is_active=True)
            # Get location names where is_active=True
            active_locations = LocationGroup.objects.filter(is_active=True).values_list('location', flat=True)
            base_query = base_query.filter(location__in=active_locations)
            print(f"Applied future CM period filter: showing {active_locations.count()} active locations")
        
        # Note: The is_active field in LocationGroup already considers all technologies appropriately
        # No special exceptions needed since the database-level active/inactive status handles edge cases
    ...
    # EGRESS OPTIMIZATION: Reduced limits to prevent large responses
    requested_limit_str = request.GET.get('limit', '250') # Reduced from 5000
    try:
        requested_limit = int(requested_limit_str)
        # Cap maximum limit to prevent egress spikes
        limit = min(500, requested_limit) if viewport_filtered else min(250, requested_limit)
    except ValueError:
        limit = 500 if viewport_filtered else 250 # Much lower fallback limits

    print(f"Using limit: {limit} (Viewport filtered: {viewport_filtered})")

    # --- Adjust fields fetched based on detail level --- START
    if detail_level == 'minimal':
        fields_to_fetch = ('id', 'latitude', 'longitude', 'technology', 'location', 'auction_name')
        ...
        fields_to_fetch = ('id', 'latitude', 'longitude', 'location',
                           'company_name', 'description', 'technology', 'delivery_year', 'cmu_id', 'auction_name')
    ...
    # Count non-geocoded components in search to help diagnose issues
    if search_query:
        geocoded_count = 0
        non_geocoded_count = 0
        for comp in components_to_render:
            if comp.get('latitude') is not None and comp.get('longitude') is not None:
                geocoded_count += 1
            else:
                non_geocoded_count += 1
        print(f"MAP DATA API: Search query '{search_query}' found {geocoded_count} geocoded and {non_geocoded_count} non-geocoded components")
    ...
    # Special handling for SW11 search
    is_sw11_search = search_query and search_query.lower().strip() == 'sw11'
    ...
    for comp in components_to_render:
        ...
        if lat is not None and lng is not None:
            ...
            # Add to coordinates map for display
            coord_key = (lat, lng)
    ...
    print(f"Found {geocoded_count} geocoded and {non_geocoded_count} non-geocoded components")
    print(f"Grouped components into {len(components_by_coord)} unique coordinates.")
    
    # For SW11 search, if we have no geocoded results, create a fallback marker
    if is_sw11_search and geocoded_count == 0 and len(components_to_render) > 0:
        print("Creating fallback marker for SW11 search with non-geocoded components")
        # Add a fallback coordinate for SW11 (Battersea area)
        sw11_coord = (51.47, -0.165)  # Approximate center of SW11
        components_by_coord[sw11_coord] = [{
            'id': 0,
            'cmu_id': 'SW11',
            'location': 'SW11 Area (Approximate)',
            'company_name': 'Multiple Companies',
            'technology': 'Various',
            'description': f'Area contains {len(components_to_render)} components without precise geocoding',
            'delivery_year': ''
        }]
        print(f"Added fallback marker for SW11 with {len(components_to_render)} non-geocoded components")
    ...
    for coord, grouped_components in components_by_coord.items():
        ...
        feature_properties = {
            ...
            'auction_name': representative_comp.get('auction_name', ''),
            'detailUrl': f'/location/by-name/{urllib.parse.quote(representative_comp.get("location", "Unknown Location"), safe="")}/' 
        }
    ...
    final_is_filtered = any([
        search_query, # Add search_query to the conditions for being filtered
    ...
    prev_stage_time = 0.0 # Initialize for the loop
    for stage, time_value in stage_times.items():
        # Calculate duration of this specific stage
        # This assumes stage_times stores cumulative time from the very start for each stage key
        stage_duration = time_value - prev_stage_time
        print(f"  {stage}: {stage_duration:.3f}s (cumulative: {time_value:.3f}s total)")
        prev_stage_time = time_value # Update prev_stage_time for the next iteration
    print("------------------------")
...
def component_map_detail_api(request, component_id):
    """API endpoint that returns detailed info for a specific component and all related components at the same location"""
    from django.core.cache import cache
    from .utils import normalize
    import logging
    import traceback
    ...
    logger = logging.getLogger(__name__)
    logger.info(f"component_map_detail_api called for ID: {component_id}")
    
    # Check for emergency mode
    emergency_mode = os.environ.get('REDIS_EMERGENCY_MODE', 'false').lower() == 'true'
    
    # Create a cache key for this component
    cache_key = f"component_detail_{component_id}"
    
    # Try to get from cache only if not in emergency mode
    if not emergency_mode:
        try:
            cached_data = cache.get(cache_key)
            if cached_data:
                logger.info(f"✅ Cache HIT: Returning cached component detail for ID: {component_id}")
                return JsonResponse(cached_data)
        except Exception as cache_error:
            logger.error(f"Error reading from cache: {cache_error}")
            # Continue with database fetch
        
        logger.info(f"⚠️ Cache MISS: Fetching component detail for ID: {component_id}")
    else:
        logger.info("🚨 EMERGENCY MODE: Skipping cache, fetching from database")
    
    # Not in cache, fetch from database
    try:
        # Get the main component
        main_component = Component.objects.get(id=component_id)
        
        # Normalize company name for the ID
        company_name = main_component.company_name or 'Unknown'
        company_id = normalize(company_name) if company_name != 'Unknown' else None
        
        # Find all components at the same location
        components_at_location = []
        
        # Safely handle the get_simplified_technology function
        try:
            display_technology = get_simplified_technology(main_component.technology)
            if not isinstance(display_technology, str):
                # Ensure it's a string for serialization
                display_technology = str(display_technology) if display_technology is not None else "Unknown"
        except Exception as tech_error:
            logger.error(f"Error getting simplified technology: {tech_error}")
            display_technology = main_component.technology or "Unknown"  # Fall back to original technology
        
        try:
            if main_component.latitude is not None and main_component.longitude is not None:
                logger.info(f"Looking for components at lat: {main_component.latitude}, lon: {main_component.longitude}, location: {main_component.location}")
                
                # Get components at the same location (with small tolerance for floating point precision)
                location_components = Component.objects.filter(
                    latitude__range=(main_component.latitude - 0.0001, main_component.latitude + 0.0001),
                    longitude__range=(main_component.longitude - 0.0001, main_component.longitude + 0.0001)
                ).order_by('-delivery_year')  # Sort by delivery year descending (newest first)
                
                # It might be better to not filter by location name to catch renamed components
                logger.info(f"Found {location_components.count()} components at same coordinates")
                
                # Create list of all components at this location
                for comp in location_components:
                    components_at_location.append({
                        'id': comp.id,
                        'delivery_year': str(comp.delivery_year or 'Unknown'),
                        'auction_name': str(comp.auction_name or ''),
                        'cmu_id': str(comp.cmu_id or 'Unknown'),
                        'derated_capacity_mw': str(comp.derated_capacity_mw or 'N/A'),  # Convert to string
                        'detailUrl': f'/location/by-name/{urllib.parse.quote(main_component.location or "Unknown Location", safe="")}/'
                    })
            else:
                logger.warning(f"Component {component_id} has no latitude/longitude coordinates")
        except Exception as loc_error:
            logger.error(f"Error finding location components: {loc_error}")
            logger.error(traceback.format_exc())
            # Continue with empty components_at_location array
        
        # Ensure we have at least the main component in the list
        if not components_at_location:
            components_at_location.append({
                'id': main_component.id,
                'delivery_year': str(main_component.delivery_year or 'Unknown'),
                'cmu_id': str(main_component.cmu_id or 'Unknown'),
                'derated_capacity_mw': str(main_component.derated_capacity_mw or 'N/A'),
                'detailUrl': f'/location/by-name/{urllib.parse.quote(main_component.location or "Unknown Location", safe="")}/'
            })
        
        # Prepare the main component data
        data = {
            'success': True,
            'data': {
                'id': main_component.id,
                'title': str(main_component.location or 'Unknown Location'),
                'technology': str(main_component.technology or 'Unknown'),
                'display_technology': display_technology,
                'company': str(company_name),
                'company_id': str(company_id) if company_id else None,
                'description': str(main_component.description or ''),
                'delivery_year': str(main_component.delivery_year or ''),
                'auction_name': str(main_component.auction_name or ''),
                'cmu_id': str(main_component.cmu_id or ''),
                'detailUrl': f'/location/by-name/{urllib.parse.quote(main_component.location or "Unknown Location", safe="")}/',
                'components_at_location': components_at_location,
                'has_multiple_years': len(components_at_location) > 1
            }
        }
        
        # Test serialization before caching
        try:
            # Verify that the data can be serialized to JSON
            json.dumps(data)
            
            # Cache the result for 30 minutes only if not in emergency mode
            if not emergency_mode:
                try:
                    cache.set(cache_key, data, 60 * 30)
                    logger.info(f"✅ Cached component detail for ID: {component_id} with {len(components_at_location)} location components")
                except Exception as cache_set_error:
                    logger.error(f"Error setting cache: {cache_set_error}")
                    # Continue even if caching fails
            else:
                logger.info("🚨 EMERGENCY MODE: Skipping cache write")
        except (TypeError, ValueError) as json_error:
            logger.error(f"Error serializing data to JSON: {json_error}")
            # Don't cache if JSON serialization fails but continue to return the response
...
@csrf_exempt
def gpt_search_api_DEPRECATED(request):
    """
    ...
    if request.method != 'POST':
        response = JsonResponse({'error': 'Only POST method allowed. Use X-API-Key header with: cmr_public_readonly_ai_access_2024'}, status=405)
        response['Access-Control-Allow-Origin'] = '*'
        return response
    
    # Check API key for authentication (support both private and public keys)
    api_key = request.headers.get('X-API-Key', '')
    private_api_key = getattr(settings, 'API_KEY', 'your_custom_api_key_for_auth')
    public_api_key = getattr(settings, 'PUBLIC_API_KEY', 'cmr_public_readonly_ai_access_2024')
    
    # Allow both private and public API keys
    if api_key not in [private_api_key, public_api_key]:
        return JsonResponse({'error': 'Invalid API key. Use X-API-Key header with: cmr_public_readonly_ai_access_2024'}, status=401)
    ...
    try:
        ...
        if not query:
            # Always return valid JSON, never "OK" - this ensures AI tools get proper responses
            return JsonResponse({
                'results_count': 0,
                'components': [],
                ...
                'meta': {
                    'api_version': '1.0',
                    'endpoint': '/api/gpt-search/',
                    'method': request.method,
                    'user_agent': request.META.get('HTTP_USER_AGENT', 'Unknown'),
                    'infrastructure_debug': True
                }
            }, status=200)  # Return 200 instead of 400 to avoid infrastructure filtering
        ...
        # Create response with CORS headers for AI systems
        ...
        response = JsonResponse({'error': 'Invalid JSON data in request'}, status=400)
        response['Access-Control-Allow-Origin'] = '*'
        return response
    except Exception as e:
...
@csrf_exempt
def gpt_api_readme(request):
    """
    Public README endpoint for the GPT Search API.
    No authentication required - provides usage instructions for AI systems.
    """
    response = JsonResponse({
        "name": "UK Capacity Market GPT Search API",
        "description": "API for searching the UK Capacity Market database using natural language queries",
        "version": "1.0.0",
        "endpoint": "POST /api/gpt-search/",
        "authentication": {
            "public_key": "cmr_public_readonly_ai_access_2024",
            "header": "X-API-Key"
        },
        "example_usage": {
            "curl": "curl -X POST https://capacitymarket.co.uk/api/gpt-search/ -H \"Content-Type: application/json\" -H \"X-API-Key: cmr_public_readonly_ai_access_2024\" -d '{\"query\": \"gas plants in London\"}'",
            "request_body": {"query": "Natural language search query"},
            "response_fields": ["query", "results_count", "summary", "components", "error"]
        },
        "example_queries": [
        ...
        "rate_limits": "Public API key is rate-limited for fair usage",
        "openapi_spec": "https://capacitymarket.co.uk/.well-known/openapi.json",
        "support": "info@capacitymarket.co.uk"
    })
    response['Access-Control-Allow-Origin'] = '*'
...
def search_json_api(request):
    """
    Public JSON API endpoint that mirrors the search-map functionality.
    No authentication required - designed to work around WAF restrictions.
    Uses GET method like the existing search-map to avoid CORS preflight.
    """
    ...
    if request.method == 'GET':
        ...
        response = JsonResponse({'error': 'Only GET and POST methods allowed'}, status=405)
        return add_cors_headers(response)
    ...
    if not query:
        ...
        response = JsonResponse({
            ...
            'components': [],
            'summary': 'No search query provided',
            'error': 'Query parameter required. Use ?q=your_search or POST with {"query": "your_search"}',
            'example_urls': [
                'https://capacitymarket.co.uk/search-json/?q=gas plants in London',
                'https://capacitymarket.co.uk/search-json/?q=battery storage',
                'https://capacitymarket.co.uk/search-json/?q=EDF nuclear'
            ],
            'meta': {
                'api_version': '1.0',
    ...
    try:
        # Use direct database search similar to search-map view
        from .models import Component
        from django.db.models import Q
        
        # Search components directly using the same logic as the working search-map
        ...
        ).order_by('company_name', 'location')[:50]  # Limit to 50 results
        ...
        response_data = {
            ...
            'api_info': {
                'endpoint': '/search-json/',
                'methods': ['GET', 'POST'],
                'parameters': 'q=search_term or {"query": "search_term"}',
                'public_access': True,
        ...
        logger.exception(f"Error in search JSON API: {str(e)}")
        response = JsonResponse({
...
def search_json_api_direct(request):
    """
    Direct JSON API endpoint that serves data without redirect.
    This is for AI systems that can't follow redirects properly.
    """
    return search_json_api(request)

def search_map_json_api_direct(request):
    """
    Direct JSON API endpoint for map search that serves data without redirect.
    This is for AI systems that can't follow redirects properly.
    """
    return search_map_json_api(request)

def redirect_to_api_subdomain(request):
    """
    Redirects old JSON API endpoints to the proper API subdomain.
    Maintains SEO best practices by having one canonical API location.
    """
    # Get the current path and query parameters
    current_path = request.path
    query_string = request.GET.urlencode()
    
    # Map old endpoints to new API subdomain paths
    path_mapping = {
        '/api/gpt-search/': '/search/',
        '/search-json/': '/search/',
        '/search-map-json/': '/search/',
    }
    
    # Find the new path
    new_path = path_mapping.get(current_path, '/search/')
    
    # Build the new URL
    api_url = f"https://api.capacitymarket.co.uk{new_path}"
    if query_string:
        api_url += f"?{query_string}"
    
    # Return 301 permanent redirect
    response = HttpResponse(status=301)
    response['Location'] = api_url
    response['Cache-Control'] = 'max-age=31536000'  # Cache for 1 year
    
    # Add helpful message for developers
    response.content = f"""
This endpoint has moved to: {api_url}

Please update your code to use the API subdomain:
- Old: https://capacitymarket.co.uk{current_path}
- New: {api_url}

The API subdomain provides:
- Better performance
...
@csrf_exempt
def search_map_json_api(request):
    """
    JSON API endpoint that mimics search-map/ behavior exactly.
    Uses the same path pattern (/search-map-*) to bypass WAF restrictions.
    """
    ...
    if not query:
        ...
        response = JsonResponse({
            ...
            'components': [],
            'summary': 'No search query provided',
            'error': 'Query parameter required. Use ?q=your_search',
            'example_url': 'https://capacitymarket.co.uk/search-map-json/?q=battery storage',
            'meta': {
                'api_version': '1.0',
                'endpoint': '/search-map-json/',
                'method': request.method,
                'user_agent': request.META.get('HTTP_USER_AGENT', 'Unknown'),
                'infrastructure_debug': True
            }
    ...
    try:
        # Create a fake request that mimics the successful search-map pattern
        factory = RequestFactory()
        fake_request = factory.get(f'/search-map/?q={query}')
        fake_request.user = request.user
        fake_request.session = getattr(request, 'session', {})
        
        # Call the working search-map view to get the context
        # This should bypass WAF since it's the same internal path that works
        from .models import Component
        from django.db.models import Q
        
        # Use the exact same database query as search-map-simple
        components_query = Component.objects.filter(
            Q(location__icontains=query) |
            Q(company_name__icontains=query) |
            Q(description__icontains=query) |
        ...
        logger.exception(f"Error in search-map JSON API: {str(e)}")
        response = JsonResponse({
            'error': f'Search failed: {str(e)}',
            'query': query,
            'components': [],
...
# -------- New Map Search View --------- START --------
@map_access_required
def map_search_view(request):
    """Render the map search page that displays search results as markers"""
    # Get search query parameters
    query = request.GET.get('q', '')
    count = request.GET.get('count', '0')
    
    technologies = (
        Component.objects.exclude(technology__isnull=True)
        .exclude(technology='')
        .values_list('technology', flat=True)
        .distinct()
        .order_by('technology')
    ...
    companies = (
        Component.objects.exclude(company_name__isnull=True)
        .exclude(company_name='')
        .values_list('company_name', flat=True)
        .distinct()
        .order_by('company_name')[:100]
    ...
    delivery_years = (
        Component.objects.exclude(delivery_year__isnull=True)
        .exclude(delivery_year='')
        .values_list('delivery_year', flat=True)
        .distinct()
        .order_by('-delivery_year')
...
# -------- New Map Search View --------- END --------

# Simple test view for SW11 search map
@map_access_required
def map_search_test(request):
    """Simple test view for SW11 search map"""
    context = {
        'api_key': settings.GOOGLE_MAPS_API_KEY,
    }
    return render(request, 'checker/map_search_simple.html', context)


def location_detail_by_name(request, location_name):
    """Find LocationGroup by name and redirect to detail view."""
    ...
    try:
        location_group = LocationGroup.objects.get(location=location_name)
    except LocationGroup.DoesNotExist:
        # Try multiple strategies to find the location
        # 1. Replace runs of spaces with newlines (common case for multi-line addresses)
        patterns_to_try = []
        
        # Pattern 1: Each space could be a newline
        # But we need to be smart about it - postcodes shouldn't have newlines
        parts = location_name.split()
        if len(parts) >= 2:
            # Try common UK address patterns
            # Usually: Name\nStreet\nCity\nCounty\nPostcode
            for i in range(1, len(parts)):
                # Don't split postcodes (last part if it matches postcode pattern)
                if i == len(parts) - 1 and re.match(r'^[A-Z]{1,2}\d{1,2}[A-Z]?\s*\d[A-Z]{2}$', parts[i], re.I):
                    continue
                test_location = ' '.join(parts[:i]) + '\n' + '\n'.join(parts[i:])
                patterns_to_try.append(test_location)
        
        # Pattern 2: Smart splitting based on common patterns
        # Try to identify where newlines should go based on common address components
        if ',' in location_name:
            # If commas exist, they might indicate line breaks
            patterns_to_try.append(location_name.replace(', ', '\n'))
        
        # Pattern 3: Look for multiple consecutive spaces as line break indicators
        patterns_to_try.append(re.sub(r'\s{2,}', '\n', location_name))
        
        # Try all patterns
        location_group = None
        for pattern in patterns_to_try:
            try:
                location_group = LocationGroup.objects.get(location=pattern)
                break
            except LocationGroup.DoesNotExist:
                continue
        
        if not location_group:
            # Final fallback: search for locations containing all the words
            words = location_name.split()
            query = Q()
            for word in words:
                query &= Q(location__icontains=word)
            
            location_groups = LocationGroup.objects.filter(query)
            if location_groups.count() == 1:
                location_group = location_groups.first()
            elif location_groups.count() > 1:
                # If multiple matches, try to find exact match ignoring whitespace differences
                normalized_input = ' '.join(location_name.split()).lower()
                for lg in location_groups:
                    normalized_db = ' '.join(lg.location.split()).lower()
    ...
    return redirect("location_detail", location_id=location_group.id)

def battery_storage_guide(request):
    """View for the battery storage guide page."""
    return render(request, 'checker/battery_storage_guide.html')

def capacity_market_guide(request):
    """View for the capacity market guide page."""
    return render(request, 'checker/capacity_market_guide.html')

def capacity_market_faq(request):
    """View for the capacity market FAQ guide page."""
    return render(request, 'checker/capacity_market_faq.html')

def secondary_trading_guide(request):
    """View for the secondary trading guide page."""
    return render(request, 'checker/secondary_trading_guide.html')

# --- END Stripe Donation Views ---

# === Test Access Donation System ===

def test_access_donation_page(request):
    """Test page for donation-based access without registration."""
    context = {
        'stripe_public_key': settings.STRIPE_PUBLIC_KEY
    }
    return render(request, 'checker/test_access_donation.html', context)

@require_http_methods(["POST"])
def create_test_access_checkout(request):
    """Creates a Stripe Checkout session for test access donation."""
    try:
        # Get amount from form (same logic as donation system)
        amount_choice = request.POST.get('amount', '500')
        custom_amount = request.POST.get('custom_amount', '')
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            line_items=[
                {
                    'price_data': {
                        ...
                        'unit_amount': donation_amount_pence,
                        'product_data': {
                            'name': f'Test Access Code - Capacity Market Search ({amount_display})',
                            'description': 'One-time contribution for test access code',
                        },
                    },
                    'quantity': 1,
                },
            ...
            mode='payment',
            success_url=base_url + reverse('test_access_success') + '?session_id={CHECKOUT_SESSION_ID}',
            cancel_url=base_url + reverse('test_access_donation_page'),
            metadata={
                'test_access': 'true'
            }
        )
        ...
        logger.error(f"Error creating test access checkout session: {e}")
        return redirect('test_access_donation_page')

def test_access_success(request):
    """Success page for test access donation with access code generation."""
    session_id = request.GET.get('session_id')
    ...
    if session_id:
        try:
            ...
            if session.payment_status == 'paid':
                ...
                # Store access code in session with expiry (24 hours for testing)
                expiry_time = timezone.now() + timedelta(hours=24)
                request.session['test_access_code'] = access_code
                request.session['test_access_expiry'] = expiry_time.isoformat()
                request.session['test_access_active'] = True
                ...
                return render(request, 'checker/test_access_success.html', context)
        except Exception as e:
            logger.error(f"Error processing test access success: {e}")
    
    # If something went wrong, redirect back to donation page
    return redirect('test_access_donation_page')

def test_access_status(request):
    """Simple page to check test access status."""
    test_access_active = request.session.get('test_access_active', False)
    access_code = request.session.get('test_access_code', None)
    expiry_iso = request.session.get('test_access_expiry', None)
    
    context = {
        'test_access_active': test_access_active,
        'access_code': access_code,
    }
    ...
    if expiry_iso:
        from django.utils import timezone
        try:
            expiry_time = timezone.datetime.fromisoformat(expiry_iso.replace('Z', '+00:00'))
            context['expiry_time'] = expiry_time
            context['is_expired'] = timezone.now() > expiry_time
            
            # Clear expired access
            if context['is_expired']:
                request.session.pop('test_access_code', None)
                request.session.pop('test_access_expiry', None)
                request.session['test_access_active'] = False
                context['test_access_active'] = False
        except:
            pass
    
    return render(request, 'checker/test_access_status.html', context)
...
# --- END Test Access Donation System ---
