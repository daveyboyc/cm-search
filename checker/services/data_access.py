...
# Import the postcode/area helper functions (try fast version first)
try:
    from .postcode_helpers_fast import get_all_postcodes_for_area, get_area_for_any_postcode
except ImportError:
...
def get_cmu_dataframe(force_rebuild=False):
    ...
    CACHE_TTL = 3600 * 24 * 1  # 7 days (to match weekly crawl frequency)
...
def save_components_to_database(cmu_id, components):
    ...
    for component in components:
        ...
        new_components.append(Component(
            ...
            cmu_id=component.get("CMU ID", cmu_id),  # Use actual CMU ID from component data, fallback to parameter
...
def get_components_from_database(cmu_id=None, component_id=None, location=None, company_name=None, search_term=None, limit=100, page=None, per_page=None, sort_order="desc", sort_by="relevance", db_filter=None, query_type='general'):
    ...
    # Initialize overall function execution timer
    start_time = time.time()
    ...
    if db_filter is not None:
        # Debug: Performance tracking for filter application
        debug_start_time = time.time()
        logger.info(f"Using provided db_filter: {db_filter}")
        
        # Add detailed debug information
        if isinstance(db_filter, Q):
            logger.debug(f"DB filter type: Q object with connector: {db_filter.connector}")
            logger.debug(f"DB filter children count: {len(db_filter.children)}")
            # Log first few children to understand filter structure
            for i, child in enumerate(db_filter.children[:3]):
                logger.debug(f"DB filter child {i}: {child}")
        else:
            logger.debug(f"DB filter type: {type(db_filter)}")
        ...
        debug_filter_time = time.time() - debug_start_time
        logger.debug(f"Time to process db_filter: {debug_filter_time:.6f}s")
    else:
    ...
    if has_filter:
        if 'final_query' not in locals():  # Only create final_query if not already set (by location search)
            # Debug: Performance tracking for query execution
            debug_query_start_time = time.time()
            logger.debug(f"Applying final filter: {filters}")
            
            # Track the SQL execution
            try:
                # First check if the query might be too complex
                children_count = len(filters.children) if hasattr(filters, 'children') else 0
                if children_count > 10:
                    logger.warning(f"Complex filter with {children_count} conditions being applied")
                
                # Apply the filter
                final_query = base_query.filter(filters)
                
                debug_filter_apply_time = time.time() - debug_query_start_time
                logger.debug(f"Time to apply filter: {debug_filter_apply_time:.6f}s")
                
                # Try to log the SQL if not too large
                if hasattr(final_query, 'query'):
                    try:
                        sql_str = str(final_query.query)
                        if len(sql_str) < 1000:  # Only log if not too large
                            logger.debug(f"Generated SQL: {sql_str}")
                        else:
                            logger.debug(f"Generated SQL length: {len(sql_str)} characters (too large to log)")
                    except Exception as sql_err:
                        logger.error(f"Error getting SQL: {sql_err}")
                
                # Count after filtering
                count_start_time = time.time()
                count_after_filter = final_query.count()
                count_time = time.time() - count_start_time
                logger.info(f"Count AFTER applying filter: {count_after_filter} (took {count_time:.6f}s)")
                
                # Record total query processing time
                total_query_time = time.time() - debug_query_start_time
                logger.info(f"Total filter processing time: {total_query_time:.6f}s")
    ...
    # Apply pagination if provided - with detailed performance tracking
    debug_pagination_start = time.time()
    if page is not None and per_page is not None:
        try:
            ...
            # Track the pre-pagination query plan if possible
            if hasattr(final_query, 'explain'):
                try:
                    # Only do this in debug mode to avoid performance impact
                    if logger.getEffectiveLevel() <= logging.DEBUG:
                        logger.debug("Query plan before pagination:")
                        explain_output = list(final_query.explain())
                        for line in explain_output[:10]:  # Limit to first 10 lines
                            logger.debug(f"  {line}")
                except Exception as explain_err:
                    logger.debug(f"Could not get query plan: {explain_err}")
            
            # Apply the pagination slicing
        ...
        logger.info(f"No pagination parameters, applying original limit: {limit}")
        final_query = final_query[:limit]
    
    debug_pagination_time = time.time() - debug_pagination_start
    logger.debug(f"Time to apply pagination/limit: {debug_pagination_time:.6f}s")
    ...
    debug_fetch_start = time.time()
    try:
        # Log the query info before execution
        logger.debug(f"Executing final query with field selection: {', '.join(fields_to_select)}")
        ...
        debug_fetch_time = time.time() - debug_fetch_start
        logger.info(f"Fetched {len(component_dicts)} component dictionaries after selection (took {debug_fetch_time:.6f}s)")
        
        # Add sample diagnostics (first result structure if available)
        if component_dicts and logger.getEffectiveLevel() <= logging.DEBUG:
            first_result = component_dicts[0]
            logger.debug(f"First result keys: {list(first_result.keys())}")
            
            # Sample values from first result (limit to a few fields)
            sample_fields = ['cmu_id', 'location', 'company_name'] 
            sample_values = {k: first_result.get(k, 'N/A') for k in sample_fields if k in first_result}
            logger.debug(f"First result sample values: {sample_values}")
    except Exception as e_values:
        debug_fetch_time = time.time() - debug_fetch_start
        logger.exception(f"Error executing final query with .values() (after {debug_fetch_time:.6f}s): {e_values}")
        logger.error(f"Error details: {traceback.format_exc()}")
    ...
    logger.info(f"PERFORMANCE SUMMARY for get_components_from_database:")
    logger.info(f"  Total execution time: {final_execution_time:.6f}s")
    logger.info(f"  Result count: {len(component_dicts)} of {total_count} total matches")
