...
CACHE_VERSION = "v2.1"
MAP_DATA_KEY_PREFIX = f"map_data_{CACHE_VERSION}:"
MAP_DATA_EXPIRATION = 60 * 60 * 24 * 1  # 2 days (reduced from 7 days to save Redis memory)
MAP_CLUSTER_KEY_PREFIX = "map_cluster:"
MAP_CLUSTER_EXPIRATION = 60 * 60 * 24 * 3  # 3 days (reduced from 14 days to save Redis memory)
MAP_DETAIL_KEY_PREFIX = "map_detail:"
MAP_DETAIL_EXPIRATION = 60 * 60 * 6  # 6 hours (component details may update)
...
def get_cached_map_data(params: Dict[str, Any]) -> Optional[str]:
    ...
    """
    # EMERGENCY: Check if map caching is disabled
    import os
    if os.environ.get('DISABLE_MAP_CACHE', 'false').lower() == 'true':
        logger.warning("Map caching disabled in emergency mode")
        return None
    ...
    try:
        ...
        logger.error(f"Redis error (likely network limit): {e}")
        return None
...
def cache_map_data(params: Dict[str, Any], data: str) -> None:
    ...
    import os
    if os.environ.get('DISABLE_MAP_CACHE', 'false').lower() == 'true':
        logger.warning("Map caching disabled in emergency mode - skipping Redis write")
        return
    ...
    try:
        ...
        logger.error(f"Failed to cache map data (Redis limit?): {e}")
