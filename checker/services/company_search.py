...
def _build_search_results(
    ...
    for company in unique_companies:
        ...
        if year_data:
            ...
            # Use pre-built CompanyLinks data for instant lookup
            has_components = False
            company_component_count = 0
            cmu_count_db = len(cmu_ids)
            
            try:
                from ..models import CompanyLinks
                
                # Try to get pre-built data
                company_links = CompanyLinks.objects.filter(company_name=company).first()
                if company_links:
                    has_components = True
                    company_component_count = company_links.component_count
                    debug_info["companies_with_components"] += 1
                    # We already have auction count from CompanyLinks
                    cmu_count_db = company_links.auction_count
                else:
                    # Fall back to quick existence check only
                    from ..models import Component
                    if Component.objects.filter(company_name=company).exists():
                        has_components = True
                        # Don't count - just mark as having components
                        company_component_count = 1  # Placeholder
                        debug_info["companies_with_components"] += 1
                    
            except Exception as e:
                logger.warning(f"CompanyLinks lookup failed for {company}: {e}")
                # Very minimal fallback - just check existence
                try:
                    from ..models import Component
                    if Component.objects.filter(company_name=company).exists():
                        has_components = True
            ...
            company_url = f'/company-list/{normalized_company_id}/'
            ...
            if not has_components:
                ...
                if company_component_count > 1:
                    component_info = f"{company_component_count} components"
                else:
                    component_info = "Has components"
...
def auction_components(request, company_id, year, auction_name):
    ...
    try:
        ...
        # Create a structure to group by exact location + description
        grouped_components = {}
        
        for component in components:
            location = component.location or "No location specified"
            description = component.description or "No description"
            
            # Create a unique key for exact location + description match
            ...
            grouped_components[group_key]["components"].append(component)
            grouped_components[group_key]["cmu_ids"].add(component.cmu_id)
            
            # Extract year from delivery_year or auction_name
            if component.delivery_year:
                grouped_components[group_key]["auction_years"].add(component.delivery_year)
            elif component.auction_name:
                # Try to extract year from auction name like "2024-25 (T-4)"
                import re
                year_match = re.search(r'(\d{4})', component.auction_name)
                if year_match:
                    grouped_components[group_key]["auction_years"].add(year_match.group(1))
        
        # Convert sets to sorted lists for display
        for group_data in grouped_components.values():
            group_data["cmu_ids"] = sorted(list(group_data["cmu_ids"]))
            group_data["auction_years"] = sorted(list(group_data["auction_years"]))
        
        logger.info(f"Organized {components.count()} components into {len(grouped_components)} location+description groups for template.")

        # Generate HTML with grouped components
        html = f"<div class='component-results mb-3'>"
        html += f"<div class='alert alert-info'>Found {components.count()} components grouped by location and description</div>"

        # Display grouped components
        for group_key, group_data in sorted(grouped_components.items(), key=lambda x: x[1]['location']):
            location = group_data['location']
            description = group_data['description']
            technology = group_data['technology']
            cmu_ids = group_data['cmu_ids']
            auction_years = group_data['auction_years']
            component_count = len(group_data['components'])
            
            # Format location as a search link
            import urllib.parse
            ...
            <div class="mb-4 border rounded p-3">
                <div class="d-flex justify-content-between align-items-start mb-2">
                    <h6 class="mb-1"><strong>{location_html}</strong></h6>
                    <span class="badge bg-secondary">{component_count} Component{'' if component_count == 1 else 's'}</span>
                </div>
                
                <div class="mb-2">
                    <span class="badge bg-info me-2">{technology}</span>
                    <small class="text-muted fst-italic">{description}</small>
                </div>
                
                <div class="d-flex flex-wrap gap-2 mb-2">
                    <div>
                        <small class="text-muted me-1">CMU IDs:</small>
            """
            
            # Add CMU ID badges with links
            for cmu_id in cmu_ids:
                cmu_detail_url = reverse('cmu_detail', kwargs={'cmu_id': cmu_id})
                html += f'<a href="{cmu_detail_url}" class="text-decoration-none me-1"><span class="badge bg-light text-dark border">{cmu_id}</span></a>'
            ...
            html += f"""
                ...
                <div class="d-flex flex-wrap gap-2">
                    <div>
                        <small class="text-muted me-1">Auction Years:</small>
            """
            
            # Add auction year badges
            for year in auction_years:
                html += f'<span class="badge bg-warning text-dark me-1">{year}</span>'
...
def company_detail(request, company_id):
    ...
    try:
        if view_mode == "year_auction":
            ...
            # NEW Logic for all_components view with grouping by location+description
            components_query = Component.objects.filter(
                company_name__in=company_name_variations
            )
            ...
            # Group components by exact location + description
            grouped_components = {}
            for component in components_query:
                ...
                if group_key not in grouped_components:
                    grouped_components[group_key] = {
                        ...
                        "component_count": 0,
                        "capacity_sum": 0,
                        "id": f"group_{len(grouped_components)}"  # Unique ID for template links
                    }
                
                # Add this component to the group
                grouped_components[group_key]["components"].append(component)
                grouped_components[group_key]["cmu_ids"].add(component.cmu_id)
                grouped_components[group_key]["component_count"] += 1
                
                # Sum capacity
                if component.derated_capacity_mw:
                    grouped_components[group_key]["capacity_sum"] += float(component.derated_capacity_mw)
                
                # Collect full auction names
                if component.auction_name:
                    grouped_components[group_key]["auction_names"].add(component.auction_name)
            
            # Convert sets to sorted lists for display and create auction-to-component mapping
            for group_data in grouped_components.values():
                group_data["cmu_ids"] = sorted(list(group_data["cmu_ids"]))
                group_data["auction_names"] = sorted(list(group_data["auction_names"]))
                
                # Create a mapping from auction name to first component with that auction
                group_data["auction_to_component"] = {}
                for component in group_data["components"]:
                    if component.auction_name and component.auction_name not in group_data["auction_to_component"]:
                        group_data["auction_to_component"][component.auction_name] = component
            
            # Convert to list and apply sorting
            grouped_list = list(grouped_components.values())
            
            # Apply sorting to groups
            if sort_field == "location":
                grouped_list.sort(key=lambda x: x["location"], reverse=(sort_order == "desc"))
            elif sort_field == "delivery_year":
                # Sort by the latest auction name in each group (lexicographically)
                grouped_list.sort(key=lambda x: max(x["auction_names"]) if x["auction_names"] else "0", reverse=(sort_order == "desc"))
            elif sort_field == "derated_capacity_mw":
                grouped_list.sort(key=lambda x: x["capacity_sum"], reverse=(sort_order == "desc"))
            else:
                # Default to location sorting
                grouped_list.sort(key=lambda x: x["location"], reverse=(sort_order == "desc"))
            ...
            context["paginator"] = paginator
            context["total_count"] = total_count
            context["grouped_count"] = len(grouped_list)
