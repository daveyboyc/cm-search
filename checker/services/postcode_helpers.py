...
def validate_postcode(postcode):
    ...
    if is_valid is None:
        try:
            ...
            cache.set(cache_key, is_valid, timeout=1800 * 24) # Cache for 24 hours
...
def get_nearest_postcodes(postcode, limit=5, radius=1000):
     ...
     if nearest is None:
         try:
             ...
             cache.set(cache_key, nearest, timeout=1800 * 6) # Cache for 6 hours
         except requests.exceptions.RequestException as e:
    ...
    if details is None:
        try:
            ...
            if result_data:
                 ...
                 cache.set(cache_key, details, timeout=1800 * 24) # Cache for 24 hours
            else:
                 details = {} # No details found
                 cache.set(cache_key, details, timeout=1800) # Cache negative result shorter
...
def get_location_to_postcodes_mapping(force_rebuild=False):
    ...
    try:
        ...
        if (len(mapping) > 0 and processed == total) or len(mapping) > 50:
            ...
            # Set a TTL cache
            cache.set(CACHE_KEY, mapping, timeout=1800*24, version=CACHE_VERSION)  # 24 hour cache
...
def get_all_postcodes_for_area(area_name):
    ...
    # FIRST: Check the static postcode lookup file (location_postcodes.json)
    try:
        from .postcode_lookup import get_outcodes_for_location
        static_outcodes = get_outcodes_for_location(area_name)
        if static_outcodes:
            logger.info(f"Found {len(static_outcodes)} outcodes in static lookup for '{area_name}': {static_outcodes}")
            cache.set(cache_key, static_outcodes, timeout=1800*48)
            return static_outcodes
    except Exception as e:
        logger.error(f"Error checking static postcode lookup for '{area_name}': {e}")
    ...
    try:
        ...
        if outcode_count > 4:  # Threshold - if it appears in >4 different post codes areas, probably not a location
            ...
            cache.set(cache_key, [], timeout=1800*24)  # Cache empty results for 24 hours
            return []
    except Exception as e:
    ...
    if not location_to_postcodes:
        # Don't rebuild - it's too expensive and causes timeouts
        # Just use an empty mapping and rely on other methods
    ...
    if area_name_lower in location_to_postcodes:
        ...
        cache.set(cache_key, postcodes, timeout=1800*48)
        return postcodes
    ...
    for known_location, postcodes in location_to_postcodes.items():
        ...
        if re.search(r'\b' + re.escape(known_location) + r'\b', area_name_lower):
            ...
            cache.set(cache_key, postcodes, timeout=1800*48)  # Cache for 48 hours
            return postcodes
    ...
    if re.match(r'^[A-Z]{1,2}[0-9]{0,2}$', outcode):  # Basic regex for outcodes
        ...
        cache.set(cache_key, [outcode], timeout=1800*48)  # Cache for 48 hours
        return [outcode]
    ...
    try:
        ...
        if found_outcodes:
            ...
            cache.set(cache_key, found_outcodes, timeout=1800*48)  # Cache for 48 hours
    ...
    cache.set(cache_key, [], timeout=1800*6)  # Cache empty results for 6 hours
    return []
