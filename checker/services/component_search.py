...
# Import fast version if available, fallback to slow
try:
    from .postcode_helpers_fast import get_location_to_postcodes_mapping, get_all_postcodes_for_area
    logger.info("Using FAST postcode helpers in component_search")
except ImportError:
    from .postcode_helpers import get_location_to_postcodes_mapping, get_all_postcodes_for_area
    logger.warning("Using SLOW postcode helpers in component_search")

# Import location search functions
try:
    from .location_group_check import should_use_location_groups
    from .location_search import search_locations
    logger.info("Location group search functions imported successfully")
except ImportError as e:
    logger.warning(f"Could not import location search functions: {e}")
    should_use_location_groups = lambda: False
...
def search_components_service(request, extra_context=None, return_data_only=False, return_json=False):
    ...
    valid_sort_fields = {
        ...
        'date': 'delivery_year', 
        'relevance': 'relevance',
        'components': 'components',  # location_search expects 'components'
        'mw': 'capacity'  # location_search expects 'capacity'
    }
    ...
    # Replace spaces and special characters for cache key safety
    cache_safe_query = normalized_query.replace(' ', '_').replace('/', '_').replace('\\', '_').replace(':', '_')
    ...
    if backend_sort_by == 'location':
        ...
        cache_key = f"search_service_{cache_safe_query}_p{page}_pp{per_page}_loc_sort_{sort_order}"
        logger.info(f"Generated Location-Specific Cache Key: {cache_key}")
    else:
        cache_key = f"search_service_{cache_safe_query}_p{page}_pp{per_page}_sort{backend_sort_by}_{sort_order}"
    ...
    # If there's a query, redirect to the optimized search v2
    from .component_search_optimized_v2 import search_components_optimized_v2
    return search_components_optimized_v2(request)
    ...
    try:
        ...
        try:
            ...
            if cmu_df is not None:
                ...
                try:
                    # Try to use CompanyLinks from database first
                    from .use_company_links import get_company_links_for_search
                    company_data_list, company_count, company_time = get_company_links_for_search(query)
                    if company_data_list:
                        # Successfully found companies using CompanyLinks
        ...
        # Check if we should use location groups
        if search_locations and should_use_location_groups():
            logger.warning(f"🚀 USING LOCATIONGROUPS for search query '{query}' (coverage above 80%)")
            use_location_groups = True
            debug_info["use_location_groups"] = True
            
            # Use location search instead of component search
            try:
                ...
                total_location_count = location_result['total_locations']
                total_component_count = location_result['total_components']
                
                # Create synthetic component data for compatibility with existing code
                # We'll handle the actual location group display in the template
                components_list_raw = []
                
                debug_info["data_access_call_type"] = "location_groups"
                debug_info["location_group_count"] = len(location_groups)
                debug_info["total_locations"] = total_location_count
                debug_info.update(location_result.get('debug_info', {}))
                
                logger.warning(f"✅ LocationGroup search SUCCESS: {len(location_groups)} groups covering {total_component_count} components")
                
            except Exception as e:
                logger.error(f"❌ Error using location groups, falling back to component search: {e}")
                logger.error(f"❌ Traceback: {traceback.format_exc()}")
                use_location_groups = False
                debug_info["location_group_error"] = str(e)
        
        # --- Conditional call to data_access based on search type --- START ---
        if not use_location_groups:
        ...
        # Skip pagination improvement logic if using location groups
        if use_location_groups:
            # LocationGroups already have correct pagination from the database
            pass
        
        # Now group the components or use location groups
        if use_location_groups:
            # We need to convert ALL LocationGroup objects from the query to dictionaries
            # First, get the full queryset from the location_result
            all_location_groups = location_result.get('location_groups', [])
            
            # Get the full queryset that was used to create the paginator
            # We need this to create proper pagination
            from ..models import LocationGroup
            
            # Recreate the query to get all matching LocationGroups
            if query:
                # Same logic as in search_locations for all queries
                component_filter = (
                        Q(location__icontains=query) |
                        Q(company_name__icontains=query) |
                        Q(description__icontains=query) |
                ...
                logger.warning(f"🔍 Found {len(matching_locations)} unique locations with matching components")
                
                # Get ALL LocationGroups for these locations
                all_location_groups_queryset = LocationGroup.objects.filter(location__in=matching_locations)
                logger.warning(f"🔍 Found {all_location_groups_queryset.count()} LocationGroups for these locations")
            else:
                # No query - get all locations
                all_location_groups_queryset = LocationGroup.objects.all()
            
            # Apply sorting
            if backend_sort_by == 'capacity':
                order_field = 'normalized_capacity_mw'
            elif backend_sort_by == 'components':
            ...
            all_location_groups_queryset = all_location_groups_queryset.order_by(order_field)
            
            # OPTIMIZATION: Paginate BEFORE converting to dictionaries
            location_group_paginator = Paginator(all_location_groups_queryset, per_page)
            try:
                location_group_page = location_group_paginator.page(page)
            except:
                location_group_page = location_group_paginator.page(1)
            
            logger.warning(f"🔍 Converting only {len(location_group_page.object_list)} LocationGroups to dictionaries (page {page} of {location_group_paginator.num_pages})")
            
            # Convert ONLY the LocationGroups on this page to dictionaries
            conversion_start = time.time()
            grouped_components = []
            for lg in location_group_page.object_list:
                # Get primary company and technology for display
                primary_company = None
                primary_tech = None
                
                # Companies is a dict with counts, get the most common one
                if lg.companies and isinstance(lg.companies, dict):
                    primary_company = max(lg.companies.items(), key=lambda x: x[1])[0] if lg.companies else None
                
                # Technologies is a dict with counts, get the most common one
                if lg.technologies and isinstance(lg.technologies, dict):
                    primary_tech = max(lg.technologies.items(), key=lambda x: x[1])[0] if lg.technologies else None
                ...
                group_dict = {
                    ...
                    'cmu_ids': lg.cmu_ids,
                    'outward_codes': [lg.outward_code] if lg.outward_code else [],
                    'counties': [lg.county] if lg.county else [],
                    'location_group_id': lg.id,  # Add ID for link to detail page
                    'normalized_capacity_mw': lg.normalized_capacity_mw,
                    'is_location_group': True  # Flag to identify this in template
                }
                grouped_components.append(group_dict)
            
            # Create a new paginator with the converted dictionaries
            # We need to maintain the same page structure but with dict objects
            from django.core.paginator import Page
            page_obj = Page(grouped_components, location_group_page.number, location_group_page.paginator)
            
            conversion_time = time.time() - conversion_start
            perf_timings["location_group_conversion"] = conversion_time
            
            logger.warning(f"✅ Using LocationGroups: {location_group_paginator.count} total, {len(grouped_components)} on page {page}")
            if grouped_components:
                logger.warning(f"✅ First group has is_location_group={grouped_components[0].get('is_location_group')}, location_group_id={grouped_components[0].get('location_group_id')}")
                logger.warning(f"✅ page_obj type: {type(page_obj)}, first item in page_obj: {type(page_obj.object_list[0]) if page_obj.object_list else 'EMPTY'}")
            ...
            debug_info["final_location_groups"] = total_location_count
            debug_info["groups_per_page"] = per_page
            debug_info["total_group_pages"] = page_obj.paginator.num_pages if page_obj else 1
        ...
        # Create a paginator for the grouped components (skip if using LocationGroups)
        pagination_start = time.time()
        
        # Set groups_per_page for all cases
    ...
    # Check if a template override was provided in extra_context
    if extra_context and 'template_override' in extra_context:
        template_name = extra_context['template_override']
    else:
    ...
    # Redirect to map view for search results with fewer than 500 components
    # Only redirect if this is not an API/AJAX call and results are geocoded
    if (not return_data_only and not return_json and 
        context.get('total_component_count', 0) > 0 and 
        context.get('total_component_count', 0) < 500 and 
        query and not request.GET.get('suppress_map', False)):
        
        from django.shortcuts import redirect
        logger.info(f"Redirecting to map_search for query '{query}' with {context.get('total_component_count')} results")
        redirect_url = f"/map_search/?q={urllib.parse.quote(query)}&count={context.get('total_component_count')}"
        return redirect(redirect_url)
