...
def show_unique_flexitricity_locations(limit=100):
    """
    Display unique locations with their most recent auction information for
    Flexitricity load curtailment/drop components, limited to the first {limit} results.
    """
    ...
    unique_norm_locations = sorted(locations_map.keys())[:limit]
    
    total_count = len(unique_norm_locations)
    print(f"Found {total_count} unique locations with Flexitricity load curtailment/drop components (showing first {limit})")
    
    # Display the data
    print("\n" + "="*140)
    print(f"{'LOCATION':<40} | {'STATUS':<10} | {'MOST RECENT AUCTION':<30} | {'MAPS':<8} | {'DESCRIPTION':<40}")
    print("="*140)
    ...
    for norm_loc in unique_norm_locations:
        ...
        try:
            ...
            # Format description
            desc = latest.description if latest.description else "N/A"
            if len(desc) > 37:
                desc = desc[:37] + "..."
            
            # Format location
            location_display = original_loc if original_loc else "N/A"
            if len(location_display) > 37:
                location_display = location_display[:37] + "..."
            
            # Format auction name
            auction = latest.auction_name if latest.auction_name else "N/A"
            if len(auction) > 27:
                auction = auction[:27] + "..."
            
            # Create a shortened Google Maps link
            encoded_location = urllib.parse.quote(original_loc)
            maps_url = f"https://www.google.com/maps?q={encoded_location}&t=k"
            maps_link = "Link"
            
            # Print the row
            print(f"{location_display:<40} | {status:<10} | {auction:<30} | {maps_link:<8} | {desc}")
    ...
    # Display summary
    print(f"\nSummary:")
    print(f"Total unique locations shown: {total_count}")
...
if __name__ == "__main__":
    try:
        show_unique_flexitricity_locations(100)
    except Exception as e:
        print(f"Error: {e}") 